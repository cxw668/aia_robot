...
TEST_CASES: list[TestCase] = [
    ...
    TestCase("个险表单", "变更投保人要下载哪份个险表单？", ["表单下载"], ["《保险合同内容变更申请书》", "保险合同内容变更申请书"]),
    TestCase("个险表单", "理赔申请要下载什么个险表单？", ["表单下载"], ["《保险金给付申请书》", "保险金给付申请书"]),
    TestCase("个险表单", "第三方代缴保费需要哪张授权委托书？", ["表单下载"], ["《第三方缴费授权委托书》", "第三方缴费授权委托书"]),
    TestCase("团险表单", "团险理赔申请书在哪里下载？", ["表单下载"], ["《团险理赔申请书》", "团险理赔申请书"]),
    TestCase("团险表单", "团险续保需要哪份申请书？", ["表单下载"], ["《团体保险续保申请书》", "团体保险续保申请书", "续保"]),
    TestCase("团险表单", "团险合同变更申请书有上海专用版吗？", ["表单下载"], ["《合同变更申请书》", "合同变更申请书", "上海专用版"]),
]
...
def evaluate_case(case: TestCase) -> dict[str, Any]:
    ...
    top_category_ok = contains_any(top_text, case.expected_categories) if case.expected_categories else True
